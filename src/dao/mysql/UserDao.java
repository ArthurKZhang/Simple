...
public class UserDao {
    ...
    public static Connection openDBConnection() {
        ...
        try {
            connection = (Connection) DriverManager.getConnection(MySqlConfig.URL, MySqlConfig.USER, MySqlConfig.PASSWD);
    ...
    public static UserBean queryUser(String userName) {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("SELECT * FROM " + MySqlConfig.TABLE_USER + " WHERE name =\"" + userName + "\"");
    ...
    public static boolean insertUser(UserBean u) {
        ...
        try {
            PreparedStatement statement = (PreparedStatement) connection
                    .prepareStatement("INSERT INTO " + MySqlConfig.TABLE_USER + " VALUES(\"" + name + "\",\"" + password + "\")");
    ...
    public static boolean updateUser(UserBean u) {
        ...
        try {
            UserBean old = null;
            Statement statement = (Statement) connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT * FROM" + MySqlConfig.TABLE_USER + "WHERE name =\"" + u.getName() + "\"");
            while (resultSet.next()) {
            ...
            String sql = "UPDATE " + MySqlConfig.TABLE_USER + " SET name=?,password=? WHERE name=? and password=?";
    ...
    public static boolean deleteUser(UserBean u) {
        String sql = "DELETE FROM " + MySqlConfig.TABLE_USER + " WHERE name=? and password=?";
        try {
            PreparedStatement preparedStatement = (PreparedStatement) connection.prepareStatement(sql);
            preparedStatement.setString(1, u.getName());
            preparedStatement.setString(2, u.getPasswd());
